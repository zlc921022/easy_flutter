...
class ApiClient {
  ...
  Future<List<MovieNews>> getNewsList() async {
    List<MovieNews> news = [];
    await http.get(provider.webUrl).then((http.Response response) {
  ...
  Future<dynamic> getNowPlayingList({int start, int count}) async {
    Response response = await provider.dio.get(AppApi.getNowPlayingList(),
  ...
  Future<dynamic> getComingList({int start, int count}) async {
    Response<Map> response = await provider.dio.get(AppApi.getCommingSoon(),
        queryParameters: {"start": start, 'count': count});
    return response.data['subjects'];
  }
  ...
  Future<dynamic> getWeeklyList() async {
    Response<Map> response = await provider.dio.get(AppApi.getWeeklyList());
  ...
  Future<dynamic> getNewMoviesList() async {
    Response<Map> response = await provider.dio.get(AppApi.getNewMovies());
    return response.data['subjects'];
  }

  // 获取北美票房榜电影
  Future<dynamic> getUsBoxList() async {
    Response<Map> response = await provider.dio.get(AppApi.getUsBox());
    List content = response.data['subjects'];
    List movies = [];
    content.forEach((data) {
      movies.add(data['subject']);
  ...
  Future<dynamic> getTop250List({int start, int count}) async {
    Response<Map> response = await provider.dio.get(AppApi.getTop250(),
        queryParameters: {'start': start, 'count': count});
    return response.data['subjects'];
  }
  ...
  Future<dynamic> getSearchListByTag({String tag, int start, int count}) async {
    Response<Map> response = await provider.dio.get(AppApi.searchByTag(),
  ...
  Future<dynamic> getSearchListByKey({String key, int start, int count}) async {
    Response<Map> response = await provider.dio.get(AppApi.searchByKey(),
  ...
  Future<dynamic> getMovieDetail(String movieId) async {
    Response<Map> response = await provider.dio.get(AppApi.getMovieDetail());
    return response.data;
  }

  // 影片剧照
  Future<dynamic> getMovieAlbum({String movieId, int start, int count}) async {
    Response<Map> response = await provider.dio.get(
        AppApi.getMovieStagePhoto(movieId),
        queryParameters: {'start': start, 'count': count});
  ...
  Future<dynamic> getActorDetail(String actorId) async {
    Response<Map> response =
        await provider.dio.get(AppApi.getActorDetail(actorId));
    return response.data;
  }
